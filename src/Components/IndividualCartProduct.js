...
function IndividualCartProduct({cartProduct, cartProductIncrease, cartProductDecrease}) {
		...
		auth.onAuthStateChanged(user => {
			if(user){
				fs.collection('Cart ' + user.uid).doc(cartProduct.ID).delete().then(() => {
					console.log('Successfully Deleted cartProduct from Firestore Database.')
				})
			}
		})
	}
	...
	return (
		<div className="product">
			...
			<div className="btn btn-danger btn-md card-btn" onClick={handleCartProductDelete}>DELETE</div>
		</div>
	)
}
